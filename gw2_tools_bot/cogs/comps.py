...
class CompSignupSelect(discord.ui.Select):
    def __init__(self, view: CompSignupView):
        self.comp_view = view
        comp_config = view.cog.bot.get_config(view.guild_id).comp
        ...
        for entry in comp_config.classes:
            ...
            if entry.required is not None:
                current_total = len(comp_config.signups.get(entry.name, []))
                remaining = max(entry.required - current_total, 0)
    ...
    async def callback(self, interaction: discord.Interaction) -> None:  # pragma: no cover - requires Discord
        ...
        message_object = interaction.message if isinstance(interaction.message, discord.Message) else None
        if message_object and comp_config.message_id != message_object.id:
            comp_config.message_id = message_object.id

        self.comp_view.cog.bot.save_config(self.comp_view.guild_id, config)

        message_updated = False
        if guild and message_object:
            embed = self.comp_view.cog._build_comp_embed(guild, comp_config, channel=channel)
            view = CompSignupView(self.comp_view.cog, self.comp_view.guild_id, channel=channel)
            try:
                if not interaction.response.is_done():
                    await interaction.response.edit_message(embed=embed, view=view)
                else:
                    await message_object.edit(embed=embed, view=view)
                self.comp_view.cog.bot.add_view(view, message_id=message_object.id)
                message_updated = True
            except (discord.Forbidden, discord.HTTPException):
                LOGGER.warning(
                    "Failed to update signup message via interaction for guild %s",
        ...
        if removed:
            response_message = f"Removed you from **{selection}**."
        else:
            emoji_text = resolve_emoji_text()
            response_message = f"Signed you up for {emoji_text}**{selection}**."
